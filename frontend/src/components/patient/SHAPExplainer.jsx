    ...
    <div className="bg-white border border-blue-100 rounded-xl p-5 shadow-lg shadow-blue-100/50 backdrop-blur-md mb-6">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-blue-100 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500">
              Log-odds contributions from the final model (research only)
            </p>
          </div>
        </div>
        ...
        >
          ...
          {isDemoMode ? 'Demo Explanation (simulated)' : 'Live Model Contributions'}
        </span>
      </div>
      ...
      <div className="space-y-4">
        {explanations.length === 0 ? (
          ...
          explanations.map((item, idx) => {
            // In demo mode the field is `contribution`; in live mode the backend returns `log_odds_contribution`.
            const contributionValue = item.log_odds_contribution ?? item.contribution ?? 0;
            ...
            const percentWidth = Math.min(
              ...
              Math.max(
                10,
                Math.round(Math.abs(contributionValue) * 100 * 2.5)
              )
            );
                ...
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500 font-mono">
                      ({typeof (item.raw_value ?? item.value) === 'number'
                        ? (item.raw_value ?? item.value).toFixed(3)
                        : (item.raw_value ?? item.value ?? 'N/A')})
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {item.direction ?? item.status ?? (isPositive ? 'increases risk' : 'decreases risk')}
                    </span>
      ...
      <div className="mt-4 pt-3 border-t border-blue-100 flex items-center gap-2 text-[11px] text-slate-500">
        ...
        <span>
          Positive values increase predicted risk (log-odds). Negative values
          decrease it. Research/hackathon benchmark — not a clinical diagnostic.
        </span>
      </div>
    </div>
  );
