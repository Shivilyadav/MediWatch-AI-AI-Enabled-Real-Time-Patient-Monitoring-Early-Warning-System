...
export function checkAndGenerateAlerts(patient, riskResult) {
  ...
  // Determine severity tier — thresholds match the model artifact's risk bands
  // (LOW < 0.3606, MODERATE >= 0.3606 < 0.7543, HIGH >= 0.7543)
  let alertSeverity = null;
  if (riskScore >= 0.7543 || news2Score >= 7) {
    alertSeverity = 'HIGH';
  } else if ((riskScore >= 0.3606 && riskScore < 0.7543) || (news2Score >= 3)) {
